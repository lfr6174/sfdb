...
<template>
  <div class="max-w-4xl mx-auto space-y-0 pb-20">
    ...
    <section class="pt-10 pb-12 md:pb-16">
      <div class="max-w-3xl">
        ...
        <div class="flex flex-wrap gap-3">
          <router-link to="/works" class="text-sm font-medium text-bg bg-primary px-5 py-2.5 hover:opacity-85 transition-opacity no-underline inline-block">
            瀏覽作品
          </router-link>
          <router-link to="/concepts" class="text-sm text-main/60 border border-main/15 px-5 py-2.5 hover:text-primary hover:border-primary/30 hover:bg-primary/5 transition-all no-underline inline-block">
            探索概念
          </router-link>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 pb-16">
      ...
      <section class="flex flex-col">
        <div class="flex items-center gap-3 mb-5">
          ...
          <button @click="refreshRandomConcept" class="flex items-center gap-1 text-xs text-main/40 hover:text-primary transition-colors whitespace-nowrap">
        ...
        <div v-if="isLoading" class="text-sm text-main/50 py-3 text-center">正在讀取作品...</div>
        <div v-else-if="works.length > 0" class="flex flex-col">
          ...
          >
            <div class="flex items-start justify-between gap-4 w-full">
              <span class="text-sm font-medium text-main group-hover:text-primary transition-colors block">
                {{ work.title }}
              </span>
            ...
            <div class="text-xs text-main/50">
        ...
        <div v-else class="text-sm text-main/50 py-3 text-center">
          目前該概念下暫無作品。
        </div>
      </section>
      ...
      <section class="flex flex-col lg:pl-10 lg:border-l border-main/10">
        <div class="flex items-center gap-3 mb-5">
          <span class="text-xs font-medium tracking-widest uppercase text-main/40 whitespace-nowrap">近期新增概念</span>
          <div class="flex-1 border-t border-main/10"></div>
        </div>

        <div class="space-y-7">
          <div v-for="(tags, category) in recentConcepts" :key="category">
            <h3 class="text-xs font-medium tracking-widest uppercase text-main/40 mb-3">{{ category }}</h3>
    ...
    <section class="">
      <div class="flex items-center gap-3 mb-5">
        <span class="text-xs font-medium tracking-widest uppercase text-main/40 whitespace-nowrap">最新資訊</span>
        <div class="flex-1 border-t border-main/10"></div>
        <router-link to="/posts" class="text-xs text-main/40 hover:text-primary transition-colors no-underline">
          查看全部
        </router-link>
      </div>
      <ul class="flex flex-col">
        <li v-for="ann in announcements" :key="ann.id">
          <router-link :to="`/posts/${ann.id}`" class="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-6 py-3.5 border-b border-main/10 last:border-0 group cursor-pointer hover:bg-primary/5 hover:-mx-4 hover:px-4 transition-colors no-underline">
            <span class="text-xs font-mono text-main/50 shrink-0 sm:w-28">{{ formatDate(ann.created_at) }}</span>
            <span class="text-sm font-medium text-main group-hover:text-primary transition-colors leading-relaxed">{{ ann.title }}</span>
          </router-link>
        </li>
      </ul>
    </section>
