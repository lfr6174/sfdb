...
# Prevents: any group-key component regressing — same-title rows differing in publisher,
# subtitle (re-editions), or source (periodicals) must stay separate — and wrong page order
def test_group_publications_keeps_distinct_editions_and_sorts_newest_first():
    ...
    grouped = _group_publications(
        [
            ...
            _pub(6, "報紙", date(2022, 9, 1), title="New", source="newspaper"),
        ]
    )

    # ids identify each expected group: plain New, gaea's New, subtitled New,
    # newspaper New, then Old, with Undated at the bottom
    assert [e["ids"] for e in grouped] == [[2], [4], [5], [6], [1], [3]]
